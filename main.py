...
def train_test(model, tra_data, tes_data, opt):
    ...
    for i in range(len(slices)):
        ...
        # predict = test_score.topk(opt.k)[1]
        # predict = predict.cpu()
        # for pred, target in zip(predict, te_out_var):
        #     hit.append(np.isin(target - 1, pred))
        #     if len(np.where(pred == target - 1)[0]) == 0:
        #         mrr.append(0)
        #     else:
        #         mrr.append(1 / (np.where(pred == target - 1)[0][0] + 1))
...
def main():
    ...
    parser.add_argument('--dataset', default='yoochoose1_64', help='dataset name: yoochoose1_64/diginetica/yoochoose1_4.')
