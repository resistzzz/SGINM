...
class SGINM(nn.Module):
    ...
    def __init__(self, opt, num_word):
        super(SGINM, self).__init__()
        self.hidden_size = opt.hidden_size
        self.n_layers = opt.n_layers
        self.embedding_size = opt.embedding_size

        self.embedding = nn.Embedding(num_word, opt.embedding_size)
        self.gru = nn.GRU(input_size=opt.embedding_size, hidden_size=opt.hidden_size, num_layers=opt.n_layers,
                          dropout=0, bidirectional=False)
        self.attn0 = Attn(hidden_size=opt.hidden_size)
        self.Linear_0 = nn.Linear(opt.embedding_size, 2 * opt.hidden_size + 128, bias=False)
        # self.Linear_0 = nn.Linear(opt.embedding_size, 2 * opt.hidden_size)
        # self.Linear_0 = nn.Linear(opt.embedding_size, 128)

        self.fc0 = nn.Linear(opt.embedding_size, 128)
        ...
        self.optimizer = optim.Adam(self.parameters(), lr=opt.lr, betas=(0.9, 0.999), weight_decay=opt.l2)
        self.scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=opt.lr_dc, gamma=0.1)
